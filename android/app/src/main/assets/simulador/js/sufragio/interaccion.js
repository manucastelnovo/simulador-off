...
function click_candidato(evento) {
    ...
    var es_blanco = false;
    if (codigo == "blanco") {
        const candidato = localController.businessData.get_candidaturas_one({
    ...
    if (categoria.max_selecciones == 1) {
        ...
        if (parts[0] == "partido") {
            ...
            if (candidato) {
                localController.seleccion.seleccionar_candidatos(categoria, [
                    codigo,
                ]);
                if (
                    ...
                    for (var i in candidato.categorias_hijas) {
                        ...
                        localController.seleccion.seleccionar_candidatos(
                            cat_hija,
                            [cand_hijo.id_umv]
                        );
                    }
...
function click_agrupacion_nivel_ubicacion(evento) {
    /*
...
function click_consulta_popular(evento) {
    ...
    setTimeout(function () {
        localController.seleccion.seleccionar_candidatos(_consulta_actual, [
            codigo,
        ]);
...
function click_pantalla_tachas(evento) {
    ...
    const _seleccion = localController.seleccion.get_seleccion();
...
function click_partido(evento) {
    ...
    var categoria = get_categoria_actual();
    setTimeout(function () {
        localController.seleccion.seleccionar_partido(codigo, categoria);
...
function click_preferencia_candidato(evento) {
    ...
    var codigo = parts[1];
    var categoria = get_categoria_actual();
    const preferencias_tmp = localController.seleccion.get_preferencias_tmp();

    var boton = evento.currentTarget;
    // Si ya esta seleccionado y vuelvo a seleccionar el mismo confirma y continua
    if (preferencias_tmp.includes(codigo) && categoria.max_preferencias == 1) {
        localController.seleccion.seleccionar_candidatos(
            categoria,
            [codigo]
        );
...
function click_tacha_candidato(evento) {
    ...
    var boton = $(evento.currentTarget);
    const tachas_tmp = localController.seleccion.get_tachas_tmp();
    localController.toggle_array_candidato(Infinity, boton, tachas_tmp, codigo);
}
...
function click_candidato_seleccionado(evento) {
    ...
    var parts = evento.currentTarget.id.split("_");
    setTimeout(function () {
        localController._cambiar_categoria(parts[1]);
        revisando = true;
    }, constants.tiempo_feedback);
}
