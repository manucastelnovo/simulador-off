...
function consulta(candidatos) {
    ...
    if (constants.asistida) {
        document.querySelector(
            "#pantalla_consulta .texto-mediano"
        ).style.display = "block";
        document.querySelector(
...
async function mostrar_voto(data) {
    if (!constants.asistida) {
        let muestra_html = constants.muestra_svg;
        const img_previsualizacion = document.getElementById(
...
function pantalla_principal() {
    ...
    document.querySelectorAll(".opcion-tipo-voto").forEach((elem) => {
...
function insercion_boleta() {
    ...
    // _votando = false;
    localController.seleccion.set_votando(false);
    document.body.setAttribute("data-state", "normal");

    if (w < 1400) {
        var src = get_path_imagen_eleccion("ingreso_boleta");
    } else {
        var src = get_path_imagen_eleccion("ingreso_boleta1920");
    }

    if (constants.asistida) {
        document.querySelector(
            "#insercion_boleta .contenedor_texto h1.titulo"
        ).innerHTML = constants.titulo;
        document.querySelector(
            "#insercion_boleta .contenedor_texto h2.subtitulo"
        ).innerHTML = constants.subtitulo;
        const subtitulo_contraste = document.querySelector(
            "#insercion_boleta .contenedor_texto h1.subtitulo_contraste"
        );
        ...
        if (w < 1400) {
            var src = get_path_imagen_eleccion("ingreso_asistida");
        } else {
            var src = get_path_imagen_eleccion("ingreso_asistida1920");
        }
        send("change_screen_insercion_boleta");
        document
            .querySelector("#img_insercion_boleta")
            .classList.add("asistida");
    }
...
function popular_pantalla_modos() {
    ...
    for (var i in constants.botones_seleccion_modo) {
        ...
        if (boton === "BTN_COMPLETA") {
            ...
            data.imagen = get_path_imagen_eleccion("votar_lista_completa");
            data.texto = "votar_lista_completa";
        } else {
            data.clase = "votar-por-categoria";
            data.cod_boton = boton;
            data.imagen = get_path_imagen_eleccion("votar_por_categoria");
...
function popular_pantalla_mensaje_final() {
    const imageNames = [
        "verificar_boleta",
        "verificar_boleta1920",
    ];
    const path_imagenes = {};
    imageNames.map(imageName => path_imagenes[imageName] = get_path_imagen_eleccion(imageName));
    return {
