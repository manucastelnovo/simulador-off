...
function popular_html() {
    return new Promise((resolve, reject) => {
        const imageNames = [
            "logo_eleccion",
            "logo_votar",
            "logo_eleccion_alto_contraste",
            "logo_votar_blanco",
        ];
        const path_imagenes = {};
        imageNames.map(
            (imageName) =>
                (path_imagenes[imageName] = get_path_imagen_eleccion(imageName))
        );
        fetch_template("encabezado", "partials")
            .then((template_header) => {
                const html_header = template_header({
                    voto: true,
                    mostrar_ubicacion: constants.mostrar_ubicacion,
                    path_imagenes: path_imagenes,
                });
                document.querySelector("#encabezado").innerHTML = html_header;
                /* Re-cargo el estado de bateria cuando termino de renderizar el template de encabezado
                por si termina despues del que el backend llame inicio_status_battery desde el document_ready */
                inicio_status_battery({batteries: currentBatteryData, modulo: constants.modulo})
                resolve();
            })
...
class Templates {
    ...
    crear_item_consulta_popular(candidato) {
        ...
        let template_data =
            this.__get_template_data_consulta_popular(candidato);
    ...
    __get_template_data_consulta_popular(candidato) {
        ...
        let _seleccion = localController.seleccion.get_seleccion();
    ...
    crear_item_partido(partido, seleccionado, categoria_actual) {
        ...
        let template_data = this.__get_template_data_partido(
            partido,
            seleccionado,
            categoria_actual
        );
    ...
    __get_template_data_partido(partido, seleccionado, categoria_actual) {
        ...
        if (constants.mostrar_fotos_candididatos_boton_partido) {
            ...
            var candidaturas =
                localController.businessData.get_candidaturas_many(filtro);
    ...
    crear_item_candidato(candidato, seleccionado, candidatos) {
        let template_name = this.__get_template_name_candidato(
            candidato,
            candidatos
        );
        var template = get_template_desde_cache(template_name);
        let template_data = this.__get_template_data_candidato(
            candidato,
            seleccionado
        );
        var item = template(template_data);
    ...
    __get_template_name_candidato(candidato, candidatos) {
        return candidato.blanco ? "candidato_blanco" : "candidato_categoria";
    }
    ...
    __get_template_data_candidato(candidato, seleccionado) {
        ...
        const nombre_lista =
            tiene_nombre_lista && typeof candidato.lista !== "undefined"
        ...
        if (
            ...
            extra_html += this.__crear_categorias_hijas(
                candidato.categorias_hijas
            );
        }
        ...
        template_data.label_titulares = constants.label_titulares;
        template_data.label_secundarios = constants.label_secundarios;
        template_data.label_suplentes = constants.label_suplentes;
        return template_data;
    }
    ...
    __crear_categorias_hijas(categorias_hijas, vista) {
        ...
        for (var l in categorias_hijas) {
            ...
            var categoria = localController.businessData.get_categorias_one({
                codigo: cat_hija[0],
            });
    ...
    generar_paneles_confirmacion(categorias) {
        ...
        for (var i in categorias) {
            ...
            html += this.__crear_item_confirmacion(
                candidato,
                categoria,
                categorias.length
            );
    ...
    __crear_item_confirmacion(candidato, categoria, cantidad_categorias) {
        let template_name = this.__get_template_name_confirmacion(
            candidato,
            categoria
        );
        var template = get_template_desde_cache(template_name);
        let template_data = this.__get_template_data_confirmacion(
            candidato,
            categoria,
            cantidad_categorias
        );
    ...
    __es_cargo_con_suplentes(cod_categoria) {
        if (
            !("cargos_con_suplentes" in constants) ||
            constants.cargos_con_suplentes === null
        )
            return false;
    ...
    __get_template_name_confirmacion(candidato, categoria) {
        ...
        function template_name_componente_confirmacion(candidato, categoria) {
            ...
            if (categoria.consulta_popular)
                return "confirmacion_consulta_popular";
    ...
    __boton_modificar_esta_activado(categoria, cantidad_categorias) {
        ...
        const es_modificar_en_categorias =
            constants.boton_modificar_en_categorias &&
            this.__modo == "BTN_CATEG";
    ...
    __get_template_data_confirmacion(
        ...
        template_data.modificar = this.__boton_modificar_esta_activado(
            categoria,
            cantidad_categorias
        );
    ...
    generar_paneles_verificacion(candidatos) {
        return new Promise((resolve, reject) => {
            ...
            fetch_template(template_name).then(
                (template_candidato_verificacion) => {
                    for (var i in candidatos) {
                        let datos_candidato = candidatos[i];
                        let candidato =
                            this.__crear_candidato_verificacion(
                                datos_candidato
                            );
                        let categoria =
                            localController.businessData.get_categorias_one({
                                codigo: candidato.cod_categoria,
                            });
                        if (categoria.adhiere == null)
                            html += this.__crear_item_verificacion(
                                candidato,
                                categoria,
                                template_candidato_verificacion
                            );
    ...
    __crear_candidato_verificacion(datos_candidato) {
        ...
        if (datos_candidato.constructor === Array) {
            ...
            let categoria = localController.businessData.get_categorias_one({
                codigo: candidato.cod_categoria,
            });
            ...
            if (datos_candidato.length > 1) {
                ...
                if (typeof categoria.max_preferencias !== "undefined") {
                    ...
                    if (
                        ...
                        id_candidatura_secundario !=
                            candidato_principal.id_candidatura
                    ) {
                        ...
                        candidato.cod_categoria =
                            candidato_principal.cod_categoria;
    ...
    __crear_item_verificacion(candidato, categoria, template) {
        let template_data = this.__get_template_data_verificacion(
            candidato,
            categoria
        );
    ...
    __get_template_data_verificacion(candidato, categoria) {
        ...
        template_data.es_consulta =
            categoria.consulta_popular && !template_data.blanco;
        template_data.categoria = categoria;
    ...
    __construir_candidatos(candidato, campo, vista) {
        var candidatos = this.__traer_candidatos_template(
            candidato,
            campo,
            vista
        );
    ...
    main_dict_candidato(candidato, id_boton, vista) {
        ...
        if (typeof candidato.secundarios !== "undefined") {
            if (!candidato.cargo_ejecutivo && candidato.codigo !== "BLC") {
                ...
                candidatos_confirmacion =
                    localController.businessData.get_candidaturas_many(filter);
                data.secundarios = this.__construir_candidatos(
                    candidato,
                    "secundarios",
                    vista
                );
        ...
        if (typeof candidato.suplentes !== "undefined") {
            data.suplentes = this.__construir_candidatos(
                candidato,
                "suplentes",
                vista
            );
...
/** Genera el mensaje de error de grabación de la boleta. */
function msg_error_validar_boleta() {
    const generador_html = (template_popup) => {
        var template_data = {
            pregunta: constants.i18n.error_validar_boleta_alerta,
            aclaracion: constants.i18n.error_validar_boleta_aclaracion,
            btn_aceptar: true,
            btn_cancelar: false,
        };
        var html_contenido = template_popup(template_data);
...
/**
 * Helper de handlebars para soportar operadores de comparación.
 */
...
function es_cargo_ejecutivo_o_preferente_unico(candidato) {
    return (
        ...
        localController.businessData.get_preferencias(candidato).length === 0
    );
}
...
function if_mostrar_data_candidato(candidato, opts) {
    return es_cargo_ejecutivo_o_preferente_unico(candidato)
        ? opts.fn(this)
        : opts.inverse(this);
}
...
function crear_div_colores(colores) {
    ...
    if (colores) {
        ...
        var template_data = {
            ...
            colores: colores,
            mostrar_color: constants.mostrar_color,
        };
