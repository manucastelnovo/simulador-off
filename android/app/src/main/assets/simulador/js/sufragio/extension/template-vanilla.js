...
class TemplateFlavor extends Templates {
    ...
    __get_template_name_lista(template_data) {
        const es_lista_completa =
            !template_data.normal && !template_data.preagrupada;
        const es_adhesion = template_data.normal && template_data.preagrupada;
    ...
    __get_template_name_confirmacion(candidato, categoria) {
        ...
        function template_name_componente_confirmacion(candidato, categoria) {
            ...
            if (
                constants.cargos_sin_suplentes &&
                constants.cargos_sin_suplentes.includes(categoria.codigo)
            ) {
        ...
        function es_candidato_gob_con_vice(categoria) {
            let gob_con_vice = constants.cargos_con_vicepresidente.includes(
                categoria.codigo
            );
            return gob_con_vice;
        }
    ...
    __get_template_data_lista(boleta, normal, preagrupada) {
        ...
        template_data.clase_solo_2_candidatos =
            candidatos.length == 2 ? true : false;
        template_data.clase_solo_1_candidatos =
            candidatos.length == 1 ? true : false;
    ...
    main_dict_candidato(candidato, id_boton, vista) {
        ...
        if (typeof candidato.secundarios !== "undefined") {
            if (
                this.__es_candidato_con_secundarios(candidato) ||
    ...
    __cargo_muestra_vice_en_verificacion(cod_categoria) {
        if (!("cargos_verificacion_con_vice_imagen" in constants)) return false;
        return (
            constants.cargos_verificacion_con_vice_imagen &&
            constants.cargos_verificacion_con_vice_imagen.includes(
                cod_categoria
            )
        );
    }
...
templateClass = new TemplateFlavor(get_modo());
